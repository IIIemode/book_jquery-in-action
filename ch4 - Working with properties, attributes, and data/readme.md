...
* Приведите примеры, когда лучше работать с атрибутами, а когда со свойствами?
...
Картинки, у которых альты уже заданы трогать не нужно.
...
Задача на отличие `.prop('checked')` и `.attr('checked')`.

## 4.4 Storing custom data on elements
Задача на отличие `data-*` атрибутов HTML и `.data(name, value)`, установленных через jQuery. Установка, изменение, последующая работа.
