...
Свойства (properties) характерны для JS-объектов, представляющих DOM-элементы.

Атрибуты (attributes) — атрибуты HTML-элемента в разметке.

Атрибуты — всегда строки, свойства могут быть любого типа.

Атрибуты хранятся в `HTMLElement.attributes`, свойства — это свойства самого DOM-объекта: `HTMLElement.className` или прототипа более высокого порядка, типа `HTMLImageElement.alt`, который наследует от `HTMLElement`.

Синхронизация значений свойств и атрибутов:
* Да — родные (встроенные) атрибуты и свойства DOM-объекта взаимосвязаны между собой.
* Нет — если родной атрибут представлен в виде булевого значения в качестве свойства, то он не синхронизируется (`input[checked]`). Как атрибут он будет `null` или равен значению (как минимум пустой строке). Атрибут не задан — `getAttribute` будет `null`, а свойство в любом случае вернет `false|true`, даже если атрибут не задан.
...
Имена свойств и атрибутов могут отличаться. Свойства всегда в камелКейсе.
